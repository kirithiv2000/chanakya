...
const createPdf = (inputHtmlFile, outputPdfFile) => phantom
  ...
  .then((page) => new Promise((resolve, reject) => {
    ...
    page.on('onError', (status) => {
      console.log("yaha aaya kya?? - 5");
    });
    // page.open("http://google.com/");
    page.open("file:///Users/rishabhverma/NavGurukul/chanakya/currentQuesPaper.html");
    console.log("inputHtmlFile", inputHtmlFile);
...
exports.createTestAndAnswerKeyPDF = async (questions, partnerName, assessmentName) => {
  const q = htmlPageGenerator(CONSTANTS.questions.pdf.quesPaperTemplate,
    { questions, partnerName, setName: assessmentName, sections: questions }, outputQuestionHtml);
  ...
  return Promise.all([q]).then(() => {
    ...
    return Promise.all([qPdf]);
  }).then(() => {
    console.log("Did it come here?? - 3")
    const pdfs = {
