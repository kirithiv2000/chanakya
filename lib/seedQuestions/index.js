...
async function UpdateToConfig(config) {
  const Config = config;
...
function getHTMLFromMDTokens(tokens) {
  const token = tokens;
  token.links = {};
...
class QuestionSeeder {
  ...
  async startParsing() {
    ...
    if (this.updateConfigFlag) {
      ...
      await UpdateToConfig(this.config);
    }
  }
    ...
    _.map(this.questions, (question, questionsIndex) => {
      _.map(question.choices, (choice, choiceIndex) => {
        ...
        _.map(['commonText', 'hiText', 'enText'], (prop) => {
          let value = getHTMLFromMDTokens(choice.tokens[prop]);
  ...
  parseQuestionFile(file) {
    ...
    const all = _.map(choices, (c, index) => {
      ...
      const choice = QuestionSeeder.getQuestionChoiceTokens(c.tokens, c.headingIndex);
  ...
  static getQuestionChoiceTokens(tokens, headingIndex) {
    ...
    const options = QuestionSeeder.parseChoiceOptions(questionChoice.tokens.options,
  ...
  static getQuestionDirValue() {
    if (process.argv.indexOf('--questionsDir') > -1) {
      const questionsDir = process.argv[process.argv.indexOf('--questionsDir') + 1];
      if (!questionsDir) {
  ...
  static parseChoiceOptions(optionTableTokens, correct) {
