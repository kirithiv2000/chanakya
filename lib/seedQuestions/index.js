...
class QuestionSeeder {
  ...
  async startParsing() {
    ...
    // Add the relevant help text in config by processing the config
    // 1. Bucket choices help Text
    _.each(this.config.buckets, (b, i) => {
      _.each(b.choices, (c, j) => {
        const helpText = this.parseHelpTextFile(c.helpTextFile);
        this.config.buckets[i].choices[j].helpText = helpText;
      })
    });
    // 2. Topic Help Files Help Text
    _.each(this.config.topicHelpFiles, (th, i) => {
      const helpText = this.parseHelpTextFile(th.file);
      this.config.topicHelpFiles[i].helpText = helpText;
    });
    ...
    // console.log(this.config.buckets[0].choices);
    // return;
    ...
    // add the help text for specific topics
    let promises = [];
    _.each(this.config.topicHelpFiles, (helpFile) => {
      let promise = axios.put(`${CONSTANTS.apiBaseUrl}/questions/topicHelpTexts/${helpFile.name}`, {
        hiHelpText: helpFile.helpText.hiText,
        enHelpText: helpFile.helpText.enText,
      });
  ...
  async addBucketsAndBucketChoices() {
    ...
    _.each(this.config.buckets, (b, i) => {
      _.each(b.choices, (c, j) => {
        ...
        let helpTextDetails = {
          hiHelpText: c.helpText.hiText,
          enHelpText: c.helpText.enText,
        };
        if (!c.id || this.ignoreConfigBucketsAndChoices) {
          ...
          promises.push(promise);
        } else if (c.id && !this.ignoreConfigBucketsAndChoices) {
          let promise = axios.put(`${CONSTANTS.apiBaseUrl}/questions/questionBuckets/${b.id}/choices/${c.id}`, helpTextDetails)
            .then((response) => {
              let choice = response.data.data;
              this.config.buckets[i].choices[j].id = choice.id;
            });
  ...
  addQuestions() {
    let allResponses = _.map(this.finalQuestions, (obj, index) => {
      if (!obj.question.id || this.addAllQuestions || this.updateConfigFile) {
  ...
  static getMDFileTokens(file) {
    const f = fs.readFileSync(file, 'utf-8');
    const tokens = marked.lexer(f);
    return tokens;
  }

  parseHelpTextFile(file) {
    const tokens = QuestionSeeder.getMDFileTokens(file);

    // Find the indexes of the Top Level (#) headings for langauge
    let langIndexes = {};
    _.map(tokens, (token, index) => {
      if (token.type === 'heading' && token.depth === 1) {
        const tokenText = token.text.toLowerCase();
        if (tokenText.includes('hindi')) {
          langIndexes.hiText = index;
        } else if (tokenText.includes('english')) {
          langIndexes.enText = index;
        }
      }
    });

    // Group the tokens of hindi & english seperately
    const helpTextTokens = {};
    langIndexes = _.pairs(langIndexes);
    langIndexes = _.sortBy(langIndexes, i => i[1]);
    _.each(langIndexes, (lang, i) => {
      const nextLang = langIndexes[i+1];
      if (nextLang) {
        helpTextTokens[ lang[0] ] = this.getHTMLFromMDTokens( tokens.slice(lang[1]+1, nextLang[1]) );
      } else {
        helpTextTokens[ lang[0] ] = this.getHTMLFromMDTokens( tokens.slice(lang[1]+1) );
      }
    });

    return helpTextTokens;
  }

  parseQuestionFile(file) {
    const tokens = QuestionSeeder.getMDFileTokens(file.fileName);
  ...
  static verifyConfigFile(questionsDir) {
    ...
    _.each(buckets, (b, i) => {
      _.each(b.choices, (file, j) => {
        let { file:qFile, helpTextFile } = file;
        qFile = `${questionsDir}/${qFile}`;
        helpTextFile = `${questionsDir}/${helpTextFile}`;
        _.each([qFile, helpTextFile], (f) => {
          if (!fs.existsSync(f)) {
            this.showErrorAndExit(`The file *${f}* does not exist.`);
          }
        });
        buckets[i].choices[j].file = qFile;
        buckets[i].choices[j].helpTextFile = helpTextFile;
        allFiles.push(qFile);
      });
    });
    ...
    // check if the topic help files exist
    const topicHelpFiles = config.topicHelpFiles;
    _.each(topicHelpFiles, (topicHelpFile, i) => {
      let { file } = topicHelpFile;
      file = `${questionsDir}/${file}`;
      if (!fs.existsSync(file)) {
        this.showErrorAndExit(`The file *${file}* does not exist.`);
      } else {
        topicHelpFiles[i].file = file;
      }
    })
...
if (!module.parent) {
  ...
  let updateConfigFlag = QuestionSeeder.getUpdateConfigFlag();
  let ignoreConfigBucketsAndChoices = QuestionSeeder.ignoreConfigBucketsAndChoices();
  ...
  // start the parsing
  let seeder = new QuestionSeeder(config, updateMdFlag, addAllQuestionsFlag, updateConfigFlag, ignoreConfigBucketsAndChoices);
  seeder.startParsing()
    .then(() => {
