...
exports.bucketChoiceSchema = Joi.object({
  ...
  helpTextFile: Joi.string().required(),
  id: Joi.number().integer().greater(0).allow(null)
    .required(),
});
...
exports.topicHelpFileSchema = Joi.object({
  name: Joi.string().valid(...CONSTANTS.questions.topics).required(),
  file: Joi.string().required(),
});
...
exports.configSchema = Joi.object({
  ...
  topicHelpFiles: Joi.array().min(1).items(exports.topicHelpFileSchema).required(),
});
