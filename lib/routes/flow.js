...
module.exports = () => ([
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        if (request.query.ngCallType === 'requestCallback') {
          ...
          const key = await studentService.sendEnrolmentKey({ mobile, partnerId });
          return {
            success: true,
            key: key.key,
          };
  ...
  },
  {
    ...
    options: {
      ...
      validate: {
        params: {
          uploadType: Joi.string().valid('answerCSV', 'questionImage', 'audioRecording'),
        },
        payload: {
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const result = await studentService.weeklyReport();
        return {
          data: result
        }
      }
