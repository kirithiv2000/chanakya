...
module.exports = class TestVersioningService extends Schmervice.Service {
  ...
  async createAndMarkAsCurrent(versionName, questionIds, buckets, txn = null) {
    ...
    const bucketIds = _.map(buckets, (b) => b.bucketId);
    const dbBuckets = await QuestionBucket.query(txn).whereIn('id', bucketIds).eager('choices');
      ...
      const promise = QuestionBucket.query().findById(b.bucketId)
        ...
        .then((choices) => {
          ...
          _.each(choices, (choice) => {
            const p = Question.query().whereIn('id', choice.questionIds).eager('options')
              .then((ques) => {
                const bucketIndex = _.findIndex(versionQuestions.buckets, (buck) => {
                  const BucketId = buck;
                  if (BucketId.id === choice.bucket.id) {
                    return true;
                  }
                  return false;
                });
                const allQuestions = _.map(choice.questionIds, (questionId) => {
                  const q = _.where(ques, { id: questionId })[0];
                  return q;
                });
