...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async myTaskReport(user, txn = null) {
    ...
    const User = user;
    const array = [];
    const tasks = await Feedback.query(txn).where('toAssign', User).eager({ 
      student: {
        partner: true,
        contacts: true,
        enrolmentKey: true,
    ...
    _.each(tasks, (task) => {
      ...
      if (ownerTask['student'].partnerAssessment) {
        ownerTask['student'].SetName = ownerTask['student'].partnerAssessment.name;
        delete ownerTask['student'].partnerAssessment;
      } else {
        ownerTask['student'].SetName = null;
        delete ownerTask['student'].partnerAssessment;
      }
      ownerTask['student'].deadline = ownerTask.deadlineAt
      ownerTask['student'].owner = ownerTask.toAssign
      ownerTask['student'].status = ownerTask.state
      ownerTask['student'].number = ownerTask['student'].contacts ? ownerTask['student'].contacts[0].mobile : null 
      ownerTask['student'].marks = ownerTask['student'].enrolmentKey ? ownerTask['student'].enrolmentKey[0].totalMarks : null
      ownerTask['student'].lastUpdated = ownerTask['student'].lastTransition ? ownerTask['student'].lastTransition['createdAt'] : null  
      array.push(ownerTask['student']);
    });
    return array;
  }
