...
module.exports = class FeedbackService extends Schmervice.Service {
  ...
  async assignFeedbackWork(payload, txn = null) {
    const { Feedback, Users, Student } = this.server.models();
    const { exotelService } = this.server.services();

    const details = payload;
    const emailId = `${details.toAssign}@navgurukul.org`
    const [ user ] = await Users.query(txn).where('email', emailId);
    const [ student ] = await Student.query(txn).where('id', details.studentId).eager({
      contacts: true,
    })
    if (exotelService.hasTemplateForStage('assignWorkSMS') === true) {
      const templateContext = {
        owner: details.whoAssign,
        assignUser: details.toAssign,
        name: student.name,
        stage: details.student_stage,
        mobile: student.contacts.length ? student.contacts[0].mobile: 'Mobile number nahi hai',
      };
      await exotelService.sendSMS(user.mobile, 'assignWorkSMS',  templateContext);
    }
