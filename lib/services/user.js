...
module.exports = class UserService extends Schmervice.Service {
  ...
  async googleLogin(idTokenWithMobile, txn = null) {
    const { Users } = this.server.models();
    const clientId = CONSTANTS.users.auth.googleClientID;
    const { idToken, mobile } = idTokenWithMobile;
    ...
    const userObj = {
      ...
      profilePic: response.payload.picture,
  ...
  async changePassword(userId, password, txn) {
    ...
    await Users.query(txn).throwIfNotFound().where({ userId }).patch({
  ...
  async findAll(txn = null) {
    ...
    _.each(allUsers, (u) => {
      if (u.email.includes('@navgurukul')) {
        ...
        alluser.push({ id: u.id, user, profilePic: u.profilePic });
      }
    });
    return alluser;
  }
  ...
  async userUpdate(userId, Details, txn = null) {
    ...
    if (details.partnerId === 0) {
      ...
      const updatePartnerUser = await Users.query(txn).update(details).where({ id: userId });
      return updatePartnerUser;
    }
    const updateUser = await Users.query(txn).update(details).where({ id: userId });
    return updateUser;
  }
  ...
  async navgurukulAdvertisement(txn = null) {
    ...
    _.each(contacts, async (contact) => {
      if (exotelService.hasTemplateForStage(smsType) === true) {
        ...
        sendSMSPromises.push(exotelService.sendSMS(contact.mobile, smsType, templateContext));
      }
    });
