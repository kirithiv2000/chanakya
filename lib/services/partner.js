...
internals.stateOppMapping = _.object(_.map(_.pairs(CONSTANTS.studentDetails.states),
  (s) => [s[1], s[0]]));
...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async create(details, txn = null) {
    ...
    const createPartner = await Partner.query(txn).insert(details);
  ...
  validateCSV(csvResults, partnerAssessment, questionSet) {
    ...
    response.data = _.map(csvResults.data, (Row, index) => {
      let row = Row;
      row = _.object(_.map(_.pairs(row), (rows) => {
        let value = rows[1];
        if (value) {
          value = value.trim();
        }
  ...
  async recordAssessmentDetails(partner, assessmentId, csvUrl, txn = null) {
    ...
    const partnerAssessment = await PartnerAssessment.query(txn).findById(assessmentId)
      .eager('partner');
    const questionSet = await assessmentService
      .getQuestionsOfQuestionSet(partnerAssessment.questionSetId);
    ...
    const promises = _.map(data.data, (stuDent) => {
      ...
      return assessmentService.recordOfflineStudentAnswers(student.data.details,
        student.data.answers, questionSet);
    });
